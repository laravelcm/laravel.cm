    ...
    @php
        ...
        function getAvatar($title): string
		{
			return 'https://ui-avatars.com/api/?name=' . urlencode($title)
                . '&background=random&bold=true&format=svg&size=512';
        }
    @endphp
            ...
            <x-sticky-content class="divide-y divide-skin-base space-y-6">
                ...
                @if($next || $previous)
                    <div class="pt-6 space-y-6">
                        @if($next)
                            <div>
                                <h2 class="text-xs leading-5 font-medium tracking-wide uppercase text-skin-base">Article
                                    suivant</h2>
                                <a href="{{ route('articles.show', $next) }}" class="mt-3 flex items-start space-x-2">
                                    <img class="h-8 w-8 object-cover shadow-lg rounded-md"
                                         ...
                                         onerror="this.onerror=null;this.src='{{ getAvatar($next->title)}}';"/>
                                ...
                                <a href="{{ route('articles.show', $previous) }}"
                                   class="mt-3 flex items-start space-x-2">
                                    <img class="h-8 w-8 object-cover shadow-lg rounded-md"
                                         ...
                                         onerror="this.onerror=null;this.src='{{ getAvatar($previous->title)}}';"/>
        ...
        <div class="lg:col-span-5">
            <header class="space-y-4">
                ...
                <a href="{{ route('profile', $article->user->username) }}" class="mt-3 shrink-0 group block lg:hidden">
                    <div class="flex items-center">
                        <div>
                            <img class="inline-block h-8 w-8 rounded-full" src="{{ $article->user->profile_photo_url }}"
                                 alt="{{ $article->user->username }}"
                                 onerror="this.onerror=null;this.src='{{ getAvatar($article->user->username)}}';">
                        </div>
                        <div class="ml-3">
                            <p class="text-sm font-medium text-skin-inverted">
                                {{ $article->user->name }}
                        ...
                        <span class="relative z-0 inline-flex shadow-sm rounded-md -space-x-px">
                            ...
                            @if($article->isNotApproved())
                                ...
                                <button onclick="Livewire.emit('openModal', 'modals.approved-article', {{ json_encode([$article->id]) }})"
                                        type="button"
                                        class="-ml-px relative inline-flex items-center px-4 py-2 border border-skin-base bg-skin-card text-sm font-medium text-green-500 hover:bg-skin-card-muted focus:z-10 focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 focus:border-green-500 focus:ring-offset-body">
                    ...
                    <div class="space-y-8 py-8 sm:flex sm:items-center sm:justify-between sm:space-y-0">
                        @if($next)
                            <div>
                                ...
                                <div class="mt-3 flex items-start space-x-2">
                                    <img class="h-10 w-10 object-cover shadow-lg rounded-md" src="{{
                                    $next->getFirstMediaUrl('media') ?? asset('images/socialcard.png') }}" alt="{{
                                    $next->slug }}"
                                         onerror="this.onerror=null;this.src='{{ getAvatar($next->title)}}';"/>
                        ...
                        @if($previous)
                            <div>
                                ...
                                <div class="mt-3 flex items-start space-x-2">
                                    <img class="h-10 w-10 object-cover shadow-lg rounded-md" src="{{
                                    $previous->getFirstMediaUrl('media') ?? asset('images/socialcard.png') }}"
                                         alt="{{ $previous->slug }}"
                                         onerror="this.onerror=null;this.src='{{ getAvatar($previous->title)}}';"/>
                                    <div class="flex flex-col space-y-1">
                                        <a class="text-base font-medium leading-4 text-skin-inverted hover:text-skin-primary-hover line-clamp-2"
    ...
    @if($article->showToc())
        <div x-data="{ openTOC: false }" class="relative lg:hidden">
            <button
                    ...
                    class="fixed z-30 right-0 top-40 flex items-center justify-center px-1.5 py-1 h-10 w-10 mt-8 -mr-1 text-skin-base border border-skin-light rounded-l-lg shadow hover:text-skin-inverted sm:mt-12 lg:hidden bg-skin-card hover:bg-skin-card-muted md:w-auto">
                                    ...
                                    <div class="px-4 sm:px-6">
                                        <div class="flex items-start justify-between">
                                            <h2 class="text-lg font-medium text-skin-inverted"
                                                id="slide-over-title">{{ __('Table des Matières') }}</h2>
                                            <div class="ml-3 h-7 flex items-center">
                                                <button type="button"
                                                        class="bg-skin-card rounded-md text-skin-muted hover:text-skin-base focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                                                        @click="openTOC = false">
