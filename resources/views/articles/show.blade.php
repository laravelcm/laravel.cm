                        ...
                        @if($next)
                            <div>
                                ...
                                <a href="{{ route('articles.show', $next) }}" class="flex items-start mt-3 space-x-2">
                                    <img class="object-cover w-8 h-8 rounded-md shadow-lg" src="{{ $next->getFirstMediaUrl('media') }}" alt="{{ $next->slug }}">
                        ...
                        @if($previous)
                            <div>
                                ...
                                <a href="{{ route('articles.show', $previous) }}" class="flex items-start mt-3 space-x-2">
                                    <img class="object-cover w-8 h-8 rounded-md shadow-lg" src="{{ $previous->getFirstMediaUrl('media') }}" alt="{{ $previous->slug }}">
        ...
        <div class="lg:col-span-5">
            <header class="space-y-4">
                ...
                <a href="{{ route('profile', $article->user->username) }}" class="block mt-3 shrink-0 group lg:hidden">
                    <div class="flex items-center">
                        <div>
                            <img class="inline-block w-8 h-8 rounded-full" src="{{ $article->user->profile_photo_url }}" alt="{{ $article->user->username }}">
                        </div>
                        <div class="ml-3">
                            <p class="text-sm font-medium text-skin-inverted">
                                {{ $article->user->name }}
            ...
            @if($media = $article->getFirstMediaUrl('media'))
                <div class="mx-auto mt-6 aspect-w-4 aspect-h-2 sm:mt-8">
                    <img class="object-cover rounded-lg shadow-lg group-hover:opacity-75" src="{{ $media }}" alt="{{ $article->title }}" />
                </div>
            @endif
            ...
            @canany([App\Policies\ArticlePolicy::UPDATE, App\Policies\ArticlePolicy::DELETE, App\Policies\ArticlePolicy::DISAPPROVE], $article)
                    ...
                    <div class="relative flex justify-center">
                        <span class="relative z-0 inline-flex -space-x-px rounded-md shadow-sm">
                            ...
                            @if($article->isNotDeclined())
                                @hasanyrole('admin|moderator')
                                    <button onclick="Livewire.emit('openModal', 'modals.declined-article', {{ json_encode([$article->id]) }})" type="button" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium border text-danger-500 border-skin-base bg-skin-card hover:bg-skin-card-muted focus:z-10 focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 focus:ring-offset-body">
