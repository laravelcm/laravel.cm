...
<li x-data="{ open: @entangle('isUpdating') }" @class(['sm:-mx-4 p-4 border border-green-500 rounded-md relative z-10' => $isSolution])>
    <div class="sm:flex sm:space-x-3" id="reply-{{ $reply->id }}">
                    ...
                    @can(App\Policies\ReplyPolicy::UPDATE, $reply)
                        ...
                        <div class="flex items-center divide-x divide-skin-base">
                            <button
                                @click="open = true"
                                type="button"
                                class="pr-2 text-sm leading-5 font-sans text-skin-base focus:outline-none hover:underline"
                            >
                                Éditer
                            </button>
                            @if (! $isSolution)
                                <button
                                    ...
                                    type="button"
                                    class="pl-2 text-sm leading-5 font-sans text-red-500 focus:outline-none hover:underline"
                                >
                                    Supprimer
                                </button>
                            @endif
                ...
                @can(App\Policies\ThreadPolicy::UPDATE, $thread)
                    @if ($isSolution)
                        <div class="mt-2 flex items-center sm:mt-0 sm:ml-4">
                            ...
                            <span class="ml-2 text-sm font-sans text-red-500 sm:hidden">
                                Retirer comme solution
                            </span>
                        </div>
                    @else
                        <div class="mt-2 flex items-center sm:mt-0 sm:ml-4">
                            ...
                            <span class="ml-2 text-sm font-sans text-green-500 sm:hidden">
        ...
        <div x-show="open" class="flex-1" style="display: none">
            ...
            @error('body')
                <p class="mt-2 text-sm text-red-500 leading-5">
                    {{ $message }}
                </p>
            @enderror
