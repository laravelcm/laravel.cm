<div x-data="wireui_datetime_picker({
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="max-h-96 overflow-y-auto p-3 sm:w-72">
        <div x-show="tab === 'date'" class="space-y-5">
            @unless ($withoutTips)
                <div class="grid grid-cols-3 gap-x-2 text-center text-skin-base">
                    <x-dynamic-component
            ...
            <div class="flex items-center justify-between">
                <x-dynamic-component
                    :component="WireUi::component('button')"
                    class="hover:bg-skin-button-hover ring-0 rounded-lg shrink-0"
                    x-show="!monthsPicker"
                ...
                <div class="w-full flex items-center justify-center gap-x-2 text-skin-base">
                    <button class="focus:outline-none focus:underline"
                            x-text="monthNames[month]"
                            x-on:click="monthsPicker = !monthsPicker"
                            type="button">
                    </button>
                    <input class="w-14 appearance-none p-0 ring-0 border-none focus:ring-0 focus:outline-none bg-skin-input"
                           x-model="year"
            ...
            <div class="relative">
                <div class="absolute inset-0 bg-skin-menu grid grid-cols-3 gap-3"
                     x-show="monthsPicker"
                     x-transition>
                    <template x-for="(monthName, index) in monthNames" :key="`month.${monthName}`">
                        <x-dynamic-component
                            :component="WireUi::component('button')"
                            class="text-skin-muted border-0 hover:bg-green-100 uppercase"
                ...
                <div class="grid grid-cols-7 gap-2">
                    <template x-for="day in weekDays" :key="`week-day.${day}`">
                        <span class="text-skin-muted text-3xs text-center uppercase pointer-events-none"
                              x-text="day">
                        </span>
                    </template>
                    ...
                    <template
                        ...
                        <div class="flex justify-center picker-days">
                            <button class="text-sm w-7 h-6 focus:outline-none rounded-md focus:ring-2 focus:ring-offset-2 focus:ring-primary-600 hover:bg-primary-100 disabled:cursor-not-allowed"
                                    :class="{
                                    'text-skin-base': !date.isDisabled && !date.isSelected && date.month === month,
                                    'text-skin-muted': date.isDisabled || date.month !== month,
                                    ...
                                    'hover:bg-primary-600 border-skin-base': date.isSelected,
                                }"
