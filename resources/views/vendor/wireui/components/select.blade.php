<div {{ $attributes->only(['class', 'wire:key'])->class('relative') }}
    ...
    <div class="relative">
                                ...
                                <div wire:ignore class="flex flex-nowrap items-center gap-1">
                                    <template x-for="(option, index) in selectedOptions" :key="`selected.${index}`">
                                        <span class="
                                            ...
                                            <button
                                                class="shrink-0 h-4 w-4 flex items-center text-secondary-400 justify-center hover:text-skin-base"
        ...
        @if ($hint)
            <label @if ($id) for="{{ $id }}" @endif class="mt-2 text-sm text-skin-base dark:text-secondary-400">
                {{ $hint }}
            </label>
        @endif
    </div>
    ...
    <x-wireui::parts.popover :margin="(bool) $label" root-class="sm:w-full">
        ...
        <div class="max-h-64 sm:max-h-60 overflow-y-auto overscroll-contain soft-scrollbar select-none"
            ...
            <div class="w-full h-0.5 rounded-full relative overflow-hidden"
                 :class="{
                    'bg--skin-card-gray': asyncData.fetching
                }">
                <div class="bg-green-500 h-0.5 rounded-full absolute animate-linear-progress"
                     style="width: 30%"
                ...
                <template x-for="(option, index) in displayOptions" :key="`${index}.${option.value}`">
                    <li tabindex="-1" :index="index">
                        <div class="px-2 py-0.5">
                            <div class="h-8 w-full animate-pulse bg-skin-card-gray rounded"></div>
                        </div>
                    </li>
                </template>
            </ul>
            ...
            @unless ($hideEmptyMessage)
                <div class="py-12 px-3 sm:py-2 sm:px-3 text-center sm:text-left text-skin-base cursor-pointer"
